    ...
    methods: {
        search: function () {
            ...
            $.ajax({
                /*后台取得数据,赋值给观察者*/
                type: "POST",
                url: "../resulthttptest/brashttplist",
                cache: false,  //禁用缓存
                data: postdata,  //传入组装的参数
                dataType: "json",
                success: function (result) {
                    console.log(result);
                    console.log("成功返回!" + typeof (result.resultBRASHttptestList));
                    console.log(result.resultBRASHttptestList);
                    console.log(result.resultBRASHttptestList.length);
                    if (result.resultBRASHttptestList.length != 0 && result.resultBRASHttptestList[0] != null) {
                        if (result.resultBRASHttptestList.length == 1) {
                            flag = 1;
                        } else {
                            flag = 0;
                        }
                        new_data.users = result.resultBRASHttptestList;
                    } else {
